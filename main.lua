...
local function toggleOtherPlayers(visible)
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= localPlayer and player.Character then
            setCharacterVisibility(player.Character, visible)
        end
    end
end

local function toggleActiveGUIs(state)
    if state then
...
local function toggleBillboards(state)
...
local function toggleChatBubbles(state)
...
-- apply to existing ScreenGuis
for _, gui in ipairs(playerGui:GetChildren()) do
    applyIgnoreInset(gui)
end

-- automatically apply to any new ScreenGuis added later
playerGui.ChildAdded:Connect(applyIgnoreInset)
...
do
    ...
    textLabel.Text = "logan's camera suite (b0.1) has loaded!"
    ...
    local fadeOut = TweenService:Create(textLabel, tweenInfo, {
        Position = UDim2.new(0.5, -200, 0.88, 0), -- returns to the starting low position
        TextTransparency = 1,
...
print("logan's camera suite (b0.1) has loaded!")
